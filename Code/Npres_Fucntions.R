...
boot.F.hat.emp.z.cv<-function(Dist.calc,x,y,z,n ,z.dim,accuracy,R = 999){
    ...
    e <- list(statistic = stats,
              ...
              data.name = paste("dimension of Z is ", z.dim,", sample size ", n, ", dimension ", d, ", bootstrap replicates ", R, sep = ""),
...
npresid.boot<-function(data.to.work,z.dim,accuracy=500,R=999){
	data.to.write.list<-data.to.work
	##############################################################################
	x<- data.to.work[,1]
	data.to.work<-data.to.work[,-1]
	...
	z<-as.matrix(data.to.work)[,1:z.dim]
	Dist.calc<-Calc.F.cond(x, y, z, z.dim,bwmet='cv.ls')
	boot.data <-boot.F.hat.emp.z.cv(Dist.calc,x,y,z,n ,z.dim,accuracy,R)
	p.value<-boot.data$p.value

	new.data<-list(boot.data=boot.data,
	p.value=p.value, data.used=data.to.write.list, cond.dist.obj=Dist.calc,'method used for bandwidth'='cv.ls')
	return(new.data)
}